...
- **✅ Cierre:** `git push` hecho por el user (origin al día hasta `09024c3`, CI verde, página Vercel OK post-push) + **APK `960d0f60` INSTALADO** en el device. La validación quedó para la próxima sesión.
- **🔴 Pendiente (retomar acá):** (1) **validar en device en 2 tandas** — tanda 1 como Pro (sin banner en dashboard · asesor/resumen/categorización IA funcionan · de paso: movimientos por fecha + filtro categoría + link privacidad en Más); tanda 2 como Free: correr `update entitlements set is_pro=false where store='manual'` (vía MCP), matar/reabrir la app → banner test al pie + 402→paywall en IA + stub de compra (esperado sin subs en Play); **al final restaurar** `is_pro=true`. El atajo dev de paywall NO existe en el APK preview (solo `__DEV__`), por eso el flip por SQL. (2) **Play Console**: subir AAB a track de testing interno + crear subs `pro_monthly` (base `monthly`, ~US$2,49) y `pro_annual` (base `annual`, ~US$19,99) — recién ahí el paywall carga packages y la compra sandbox puebla `entitlements` vía el webhook ya verificado. (3) Menores: íconos definitivos, interstitial/rewarded (JS puro, OTA-eable post-rebuild). Queda 1 commit docs local sin pushear tras este cierre.
...
*Última actualización: 2026-06-11 (cierre sesión 14: privacy policy live + webhook RC verificado end-to-end + push a origin + APK `960d0f60` instalado; falta validación en device (2 tandas, flip Pro por SQL) + Play Console). Historial detallado por sesión/sprint en el git log.*
