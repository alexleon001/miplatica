# Mi Plata (ex "Mi Platica") — Estado del Proyecto

> App móvil de finanzas personales con IA, multi-país AR (ARS/USD, inflación, MEP/Blue/CCL, FCI, CEDEARs) + VE (Bs/USD, BCV/paralelo).
> Tagline: *"Tu plata, con inteligencia."* — Renombrada a **Mi Plata** en s20; los identificadores internos (slug `mi-platica`, scheme `miplatica`, package `com.alexleon001.miplatica`, keys AsyncStorage) NO cambian nunca.
...
**Web:** `https://miplatica.vercel.app/` = landing SEO (s20) · **Privacy policy:** `https://miplatica.vercel.app/privacidad.html` (Vercel proyecto `miplatica`, sirve `web/`; redeploy automático en cada push a `main`; si algún panel externo — AdMob/Play — tiene registrada la URL raíz como privacy, actualizarla a `/privacidad.html`)

---

## Estado actual — sesión 21 (2026-07-28)

**Foco s21: requisitos duros de Google Play (la cuenta de Play Console ya está aprobada).**

- **Borrado de cuenta end-to-end (requisito Play, era el hueco más grave):** pantalla
  `app/delete-account.tsx` (entrada en Más → Sesión, hay que tipear "ELIMINAR", avisa que la sub Pro
  se cancela aparte en Play) + edge **`delete-account` v1 ACTIVE** (`verify_jwt=true`, el uid sale del
  JWT) + página pública **`https://miplatica.vercel.app/eliminar-cuenta`** (la URL que pide el form de
  Data safety) enlazada desde la privacy. **Migración `0022` aplicada**: `shared_expenses.created_by` y
  `settlements.created_by` eran `not null` + `on delete set null` → el borrado explotaba; ahora
...
- **Cuenta de revisión para "App access":** usuario AR `miplata.review.ar@gmail.com` con Pro manual y
  ...
  `memory/project_play_reviewer_account.md`, NO acá (repo público).**
...
- **🔴 Pendiente s21+ (lo corre el user):** (1) **closed testing 12 testers × 14 días** si la cuenta es
  personal — subir el AAB `703c2509` y arrancar el reloj YA · (2) **screenshots** AR y VE en device ·
  (3) Play Console: ficha con `docs/store-listing.md`, App content con `docs/play-data-safety.md`,
  subs `pro_monthly`/`pro_annual` + merchant + service account → RevenueCat · (4) Supabase **Pro**
  (Free se auto-pausa = app muerta durante la review) · (5) Auth → URL Configuration: Site URL
  `https://miplatica.vercel.app` + redirect `/**` · (6) secrets Resend · (7) units AdMob
  interstitial/rewarded + SSV (mig 0014) · (8) validar en device el backlog s7–s21.

## Estado anterior — sesión 20 (2026-07-16)

**Foco s20: reactivación Supabase + hardening + REBRAND "Mi Plata" + landing/ASO (arranque del lanzamiento AR+VE).**

- **Ops Supabase:** el proyecto estaba AUTO-PAUSADO (Free pausa por inactividad ~1 semana; la app deja de funcionar mientras tanto). Restaurado; `centros-acopio-venezuela` pausado para liberar el slot (límite Free = 2 proyectos activos). Datos refrescados a mano (rates AR+VE del día, IPC junio). **Gotcha operativo:** sin tráfico regular se re-pausa; para lanzar en serio considerar Pro o tráfico real.
- **Migración `0019_security_hardening` aplicada:** revoca EXECUTE de `anon` (y de `authenticated` en las internas) sobre las SECURITY DEFINER expuestas + `search_path` en `set_updated_at`. Cierra los WARNs del advisor. Quedan (menores, no SQL): leaked password protection OFF (toggle en dashboard Auth) y `pg_net` en schema public (no mover: los crons dependen).
- **REBRAND → "Mi Plata"** (decidido por el user; "platica" sonaba mexicano): display name en `app.json`, copy de login/onboarding/paywall/upsell/ProLock/groups/share-texts, personas IA. Edges `financial-advisor` (v15) y `monthly-summary` (v8) redeployadas con el nombre nuevo; `categorize-*`/mail edges tienen el string viejo en prod hasta su próximo deploy (irrelevante: no visible). Commits `2e208b0` + `e873a37`.
- **Web (s20):** `web/index.html` = landing con SEO real (hero, features, secciones AR/VE, FAQ, JSON-LD MobileApplication) · `web/privacidad.html` = política renombrada (in-app link de Más actualizado). Sale a Vercel con el próximo push a `main` (lo corre el user).
- **ASO:** `docs/store-listing.md` = ficha Play Console lista (título 25 chars, short desc y long desc variantes AR/VE, keywords, release notes). Faltan assets: ícono definitivo, feature graphic, screenshots por país.
- **Usuario VE de prueba (s20):** creado y sembrado (país VE, dólar paralelo, **Pro manual**, 3 cuentas Banesco Bs / efectivo USD / USDT + 5 movimientos julio). **Credenciales en `memory/project_ve_test_user.md`, NO acá (repo público y el tester tiene Pro → IA).** **Backend VE validado end-to-end por API**: login OK + `financial-advisor` respondió en venezolano con los datos del tester y tasa paralelo. Al crearlo se encontró y arregló el bug del check (mig `0020`). Falta la pasada visual en device con este login.
- **Entregas:** OTAs `2acef385`+`13fbdddd` a `preview` (rebrand + link privacidad) · APK preview `9546be56` con nombre nativo "Mi Plata" · **AAB de PRODUCCIÓN `1ce60f68`** (primer bundle apto Play Store; artifact en expo.dev/…/builds; banner unit real ya en EAS env production, interstitial/rewarded apagados hasta crear units). Landing v2 con mockups CSS de la app LIVE (verificada con Playwright desktop+mobile). `web/app-ads.txt` (pub-9459018521108034) + `docs/play-data-safety.md` (respuestas pre-armadas de App content) listos.
- **🔴 Pendiente s20+ (camino a Play Store, ~3-4 semanas):** (1) user crea cuenta Play Console (US$25; ⚠️ cuentas personales nuevas exigen **closed testing ~12 testers × 14 días** antes de producción — crearla YA) · (2) instalar APK `9546be56` y **validar en device** (VE + Linea + splits + rebrand, backlog s7–s19) · (3) assets: ícono definitivo 512², feature graphic 1024×500, screenshots por país · (4) Play Console: subs `pro_monthly`/`pro_annual` + merchant + service account → RevenueCat, formularios con `docs/play-data-safety.md`, custom listings AR/VE con `docs/store-listing.md`, subir AAB `1ce60f68` a closed testing · (5) units AdMob interstitial/rewarded + SSV (mig 0014) · (6) secrets Resend · (7) Supabase Pro para lanzamiento (Free auto-pausa) · dominio propio (¿miplata.app?).

## Estado anterior — sesión 19 (2026-06-22)

**Foco s19: VERSIÓN VENEZUELA vía arquitectura multi-país (un solo binario/backend). Ciclo F1–F6 COMPLETO a nivel código + backend; falta OTA + validar en device.**

- **Decisiones:** una sola app multi-país (país en onboarding, NO build separado) · buscar equivalentes VE · cripto USDT+BTC (ya soportado). Principio: las columnas `_ars` son el **slot de moneda local** (ARS en AR, VES en VE); no se migran datos, `profiles.country` decide rótulo/tasas/instrumentos/features.
- **`lib/countries.ts`** = fuente de verdad por país: locale, currencyCode/Label/Symbol, usdTypes, ratesEndpoint, features {inflation, mercadoPago}, instruments. AR=todos; VE=`{usd_cash, crypto}`, dólares BCV/paralelo, sin inflación/MP.
- **Fuentes VE:** cotizaciones `ve.dolarapi.com` (BCV oficial + paralelo, valor en `promedio`) ✅. Inflación: OVF publica INPC pero SIN API → feature inflación APAGADA en VE. Pagos: sin API → MP oculto.
- **Migraciones aplicadas s19:** `0017_profile_country` (col `country` default 'AR' check AR/VE), `0018_exchange_rates_ve` (cols `bcv`/`paralelo`).
- **Edges (prod):** `fetch-exchange-rates` v10 multi-país (AR dolarapi + VE ve.dolarapi, merge en la fila del día, best-effort) · `financial-advisor` + `monthly-summary` con personas AR/VE (`PERSONA_AR`/`PERSONA_VE`, VE en tuteo venezolano sin inflación) elegidas por `profile.country`; contexto rotula Bs + cotizaciones VE.
- **Cliente:** store `currency` con `country`/`setCountry` (`UsdType`=`RateKey`) · `_layout` espeja country del perfil · onboarding/edit-profile selector de país · MoneyAmount/CurrencyToggle/ExchangeRatesBar/AccountsList/add-account/add-investment conscientes de país · `useLocalUsdRate()`+`rateForUsdType()` (reemplazan `rates?.mep` en net-worth/projection/investments/sim) · `freshenUsdValue()` recalcula valor local de posiciones USD = USD×tasa-país (arregla crypto/VES inflado por el cron AR) · add-investment filtra instrumentos por país · more.tsx oculta MP+simulador en VE · investments gatea rendimiento real por país.
- **Verde:** type-check + lint (0 err) + 142 tests. Cambio de comportamiento AR: usuarios con dólar≠MEP ahora ven proyección/inversiones/patrimonio con SU tasa (el user usa MEP → sin cambios para él).
- **🔴 Pendiente s19:** OTA a preview + **validar en device** flujo VE (país=Venezuela → Bs/BCV/paralelo, instrumentos usd_cash+crypto, asesor venezolano, sin MP/inflación). Menor: rate-alerts sigue con dólares AR (no gateado). Detalle en `memory/project_venezuela_multicountry.md`.

## Estado anterior — sesión 18 (2026-06-19)

**Foco s18: Gastos compartidos (split estilo Splitwise), PRIMER dominio multi-usuario.** Código completo + server-side aplicado/deployado. Resumen operativo (detalle en git):

- **Modelo:** participantes híbridos (invitás por email → miembro real si baja la app, si no "fantasma"; se vincula al loguearse con ese email vía `claim_group_invites`). Free = 1 grupo activo (enforce en `create_expense_group`). Recordatorios por mail (Resend). Saldar puede registrar movimiento real (ledger separado).
- **Backend (mig `0015`/`0016` ✅ aplicadas):** 5 tablas (`expense_groups`/`group_members` con user_id null=fantasma/`shared_expenses`/`expense_splits`/`settlements`). RLS por **membresía** vía `is_group_member`/`is_group_owner` security definer (evita recursión). `my_group_balances` (saldo por grupo). Lógica pura `lib/splits.ts` (split equal/exact/weights, `simplifyDebts`). Stack `app/groups/` + modales + entrada en Más + card dashboard.
- **Email (Resend):** `_shared/email.ts` + edges `group-invite`/`group-reminder`/`split-reminders-cron` (cron `[0 12 * * *]`) DEPLOYADAS.
- **🔴 Pendiente:** (1) **setear secrets** `RESEND_API_KEY`/`RESEND_FROM` (crear cuenta Resend; sin key las edges dan 502 pero NO rompen el alta). (2) **validar en device** flujo híbrido + splits + settle + mail. OTA ya en `preview` (`b5a48d65`).

## Estado anterior — sesión 17 (2026-06-16)

**Foco s17: validación de la capa de monetización en device — TODO VERDE.** Tanda Pro (sin ads, IA OK) + tanda Free (banner test, interstitial al 4° movimiento, rewarded→crédito IA + chip "usos gratis") sobre OTA `82828663`/APK `960d0f60`; `entitlements` Pro `manual` flipeado a `false` y restaurado a `true` por MCP al cerrar. Hallazgo UX: el `AdBanner` solo vive en el dashboard al final del scroll (`app/(tabs)/index.tsx:77`) → mejora opcional = pinearlo arriba del tab bar. **Próximo:** Play Console (subs) · units reales AdMob · activar SSV.

### s15–s16 (rewarded→IA + interstitials; detalle en git log)

- **✅ Crédito de IA por rewarded ad — end-to-end.** Migración `0013_ai_reward_credits` aplicada (`grant_ai_reward_credit()`/`consume_ai_reward_credit()`, tope 3/día y 3 acumulados; **sin SSV** → costo topeado a centavos) + 4 edges de IA con `ai-gate` que consume crédito antes del 402. Cliente: `use-reward-credits` + `ProLock` ("mirá un anuncio y usala una vez") + gating en `advisor`/`monthly-summary` + `RewardCreditsChip`. OTA-safe; en production OCULTO (unit rewarded sin setear), visible en `preview` (unit `test`).
- **⏳ SSV de rewarded — foundation SIN aplicar:** migración `0014_ai_reward_credit_ssv` (`grant_ai_reward_credit_for(uuid)` service-role) + edge `rewarded-ssv` (verifica firma ECDSA de AdMob). No testeable con test units. **Activar (con unit real):** (1) apply `0014`; (2) deploy `rewarded-ssv` (`verify_jwt=false`); (3) pegar URL como SSV callback en AdMob; (4) `setServerSideVerificationOptions({userId})` en `lib/rewarded.ts`; (5) **quitar** el `grant_ai_reward_credit()` client-side (si no, doble crédito); (6) validar contra callback real.
- **✅ Interstitials con tope** (`lib/ad-frequency.ts`: 1 cada 4 acciones, mín 4 min; `maybeShowInterstitial(isPro)`, trigger = guardar movimiento). Env ads `_INTERSTITIAL_/_REWARDED_ANDROID`=`test` en `.env`/`preview`, **production sin setear**.

## Historial operativo condensado (sesiones 8–14; detalle en git log)

- **s14 (2026-06-11):** privacy policy LIVE en `https://miplatica.vercel.app/` (Vercel proyecto `miplatica`, integración GitHub: push a `main` redeploya; gotcha `.vercelignore`: usar `/*`+`!/web`, la forma `*`+`!web/**` NO re-incluye) + link in-app al pie de Más (`16828d8`) · secret `REVENUECAT_WEBHOOK_SECRET` verificado (probe → 401) · **webhook RC end-to-end OK** (el header Authorization del panel estaba VACÍO, el user lo cargó; test event → 200; secret en `memory/project_monetization.md`, NO acá: repo público) · APK `960d0f60` (runtime `0.1.0`, expira 2026-06-25, primero con ads/IAP nativos) instalado en device.
...
- **✅ Monetización validada en device (s17, 2026-06-16):** tanda Pro (sin ads, IA OK) + tanda Free (banner test ✓, interstitial al 4° movimiento ✓, rewarded→crédito IA + chip "usos gratis" ✓) sobre OTA `82828663`/APK `960d0f60`; Pro `manual` restaurado a `true` al cerrar. **Hallazgo UX:** el `AdBanner` vive solo en el dashboard, al final del scroll (`app/(tabs)/index.tsx:77`) → un Free que no scrollea casi no lo ve; mejora opcional pendiente = pinearlo arriba del tab bar y/o sumarlo a más tabs.
- **🔴 Backlog de validación en device (s7–s13, casi nada corrido):** ~~monetización~~ ✅ (s17) · s11 (movimientos agrupados por fecha + filtro categoría, empty states con CTA, card "Primeros pasos") · s9 (alertas de cotización, insights, simulador, categorías custom) · s8 (proyección avanzada, alertas de presupuesto 80/100%, resumen mensual IA, sparkline, export CSV, recurrentes, prefs notif, fix FABs) · s7 (refresh visual, selector FCI real, saldo acumulado + aviso "te quedás sin efectivo en X") · previos (camino feliz CRUD, import CSV, MP OAuth en device, "real +Y%", deep-links de notif).
...
- **Multi-país AR/VE** (s19, ⏳ falta OTA + device): país en onboarding (`profile.country`). `lib/countries.ts` decide moneda local (slot `_ars`: ARS/VES, símbolo $/Bs.), cotizaciones (AR oficial/mep/blue/ccl/tarjeta vía dolarapi; VE BCV/paralelo vía ve.dolarapi), instrumentos (VE=usd_cash+crypto), persona IA y features (VE sin inflación/MP). `useLocalUsdRate`/`rateForUsdType` para conversión por país; `freshenUsdValue` corrige valor local de posiciones USD.
- **Gastos compartidos** (s18, ⏳ pendiente aplicar server-side + validar): grupos para dividir gastos (viaje/convivencia/salida), multi-usuario. Miembros reales o "fantasma" (híbrido). Gastos con split equal/exact/shares/percent (`lib/splits.ts`), balances "quién le debe a quién" (`simplifyDebts`), settle con opción de registrar movimiento, invitaciones + recordatorios por mail (Resend), historial. Stack `app/groups/` + entrada en Más + card en dashboard. Free = 1 grupo activo (Pro ilimitado, enforce en `create_expense_group`).
...
| `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ANDROID` / `_REWARDED_ANDROID` | cliente `.env` + EAS env | ✅ `preview`/`.env`=`test`; `production` SIN setear (crear units reales en AdMob primero; sin valor = capa apagada) |
| `RESEND_API_KEY` | **Edge Functions** (`supabase secrets set`) | ⏳ SIN setear (gastos compartidos: invitaciones/recordatorios por mail; sin key las edges fallan cerrado y no mandan nada). Crear cuenta Resend. |
| `RESEND_FROM` | **Edge Functions** | ⏳ SIN setear (remitente; default `onboarding@resend.dev` para pruebas, dominio verificado para prod) |
...
- [x] **Gastos compartidos** (s18): edges `group-invite` (v1, jwt) · `group-reminder` (v1, jwt) · `split-reminders-cron` (v1, verify_jwt=false, cron diario `[0 12 * * *]`) — todas usan `_shared/email.ts` (Resend). Migraciones `0015`/`0016` aplicadas. ⏳ Solo falta setear secrets `RESEND_API_KEY`/`RESEND_FROM`.

**Migraciones aplicadas:** `0001_init_schema`, `0002_helper_views`, `0003_refresh_positions`, `0004_schedule_crons`, `0005_budget_spent_triggers`, `0006_inflation`, `0007_mp_connections` (cifrada con pgcrypto + `mp_oauth_states` + funciones `mp_store_connection`/`mp_get_tokens` con `search_path = public, extensions` — pgcrypto vive en `extensions`), `0008_portfolio_by_type` (vista `v_portfolio_by_type`), `0009_payment_projection` (`projection_items` + `projection_income`, RLS owner-only), `0010_schedule_mp_sync_cron`, `0011_projection_item_interest` (columna `interest_rate numeric(6,2)` nullable), `0012_entitlements` (✅ **aplicada 2026-06-09** — Pro/IA: tabla `entitlements` + `is_pro()` + `ai_usage_daily` + `consume_ai_quota()`), `0013_ai_reward_credits` (✅ **aplicada 2026-06-13** — créditos de IA por rewarded ad: tabla `ai_reward_credits` + `grant_ai_reward_credit()` + `consume_ai_reward_credit()`, tope 3/día y 3 acumulados), `0014_ai_reward_credit_ssv` (⏳ **NO aplicada** — `grant_ai_reward_credit_for(uuid)` service-role-only para SSV; aplicar recién al activar SSV), `0015_expense_groups` (✅ **aplicada s18** — gastos compartidos: 5 tablas + RLS por membresía + `is_group_member`/`is_group_owner`/`create_expense_group`/`claim_group_invites`/`my_group_balances` + `handle_new_user` extendido), `0016_schedule_split_reminders` (✅ **aplicada s18** — cron diario `split-reminders-cron` `[0 12 * * *]`), `0017_profile_country` (✅ **aplicada s19** — `profiles.country` default 'AR' check AR/VE; multi-país), `0018_exchange_rates_ve` (✅ **aplicada s19** — cols `bcv`/`paralelo` en `exchange_rates`; cotizaciones VE), `0019_security_hardening` (✅ **aplicada s20** — revoke EXECUTE anon/public de las SECURITY DEFINER + search_path en `set_updated_at`), `0020_profile_usd_type_ve` (✅ **aplicada s20** — bug multi-país: el check de `preferred_usd_type` no permitía `bcv`/`paralelo`; ampliado a ambos países), `0021_profile_name_nullable_onboarding` (✅ **aplicada 2026-07-23** — `handle_new_user` deja `name` NULL si no viene en la metadata → vuelve a dispararse el onboarding; el archivo se versionó recién en s21), `0022_account_deletion` (✅ **aplicada s21** — `created_by` nullable en `shared_expenses`/`settlements`: sin esto `auth.admin.deleteUser()` fallaba y el borrado de cuenta que exige Play era imposible).
...
| 10 | Monetización (freemium ads + Pro con IA, RevenueCat) + fixes device | 🚧 Fase 1 ✅ (s12) · Fase 2 código+build+webhook ✅ + privacy policy live (s14) + interstitials (s15) + crédito rewarded→IA end-to-end ✅ (s16) + **validado en device ✅ (s17)**; falta subs en Play Console + units reales AdMob + activar SSV |
...
| 13 | Gastos compartidos (split estilo Splitwise): multi-usuario, RLS por membresía, splits, balances, settle, invitaciones + recordatorios por mail (Resend); Pro con 1 grupo gratis | 🚧 código + server-side completos s18 (migraciones `0015`/`0016` aplicadas, 3 edges ACTIVE, types regenerados, `lib/splits`, hooks, stack `app/groups/`) · ⏳ falta setear secrets Resend + validar en device |

*Última actualización: 2026-07-28 (sesión 21: requisitos duros de Play — borrado de cuenta end-to-end (pantalla + edge `delete-account` + página `/eliminar-cuenta` + migración 0022, probado con usuarios descartables), assets definitivos de marca vía `scripts/gen-play-assets.py` (ícono 512 + feature graphic), cuenta de revisión AR sembrada para App access, `expo-navigation-bar` para los botones del sistema, AAB de producción `703c2509` vc5 en build, web live con /eliminar-cuenta + /app-ads.txt + /restablecer. Falta: closed testing 12×14, screenshots en device, formularios y subs en Play Console, Supabase Pro. Sesión 20: reactivación Supabase (auto-pausado; centrosVE pausado para liberar slot Free), migración 0019 de hardening aplicada, REBRAND a "Mi Plata" con tagline "Tu plata, con inteligencia." (identificadores internos intactos), landing SEO en la raíz de miplatica.vercel.app + privacy movida a /privacidad.html, ASO drafts en docs/store-listing.md, OTA rebrand a preview + APK con nombre nativo nuevo. Falta: push del user, validar en device, Play Console. Sesión 19: VERSIÓN VENEZUELA vía arquitectura multi-país — un solo binario/backend, país en onboarding. Ciclo F1–F6 completo a nivel código + backend: `lib/countries.ts`, `profiles.country` (mig 0017), cotizaciones VE BCV/paralelo (mig 0018 + edge `fetch-exchange-rates` v10 multi-país), conversión por país (`useLocalUsdRate`/`freshenUsdValue`), instrumentos filtrados (VE=usd_cash+crypto), IA venezolana (`financial-advisor`/`monthly-summary` deployados), MP/inflación ocultos en VE. Type-check + lint + 142 tests verdes. ⏳ Falta: OTA a preview + validar en device. Detalle en memory/project_venezuela_multicountry.md. Sesión 18: feature grande NUEVA — Gastos compartidos / split de gastos, primer dominio multi-usuario. Código completo Fases 1–5: migraciones `0015`/`0016`, `lib/splits.ts` +17 tests, hooks, stack `app/groups/`, 3 edges de mail Resend. Type-check + lint limpios, 142 tests verdes. Server-side aplicado vía MCP (migraciones 0015/0016, 3 edges ACTIVE, types regenerados). ⏳ Falta: secrets Resend + validar en device. Sesión 17: capa de monetización VALIDADA en device end-to-end — Pro sin ads + Free con banner/interstitial/rewarded→crédito IA; Pro manual restaurado. Próximo: Play Console subs + units reales AdMob + SSV). Sesión 16: puente rewarded→IA completo end-to-end — el server-side ya estaba live (migración 0013 + 4 edges con el gate de crédito), se cableó el cliente (commit `2c186f6`: `use-reward-credits` + ProLock con "mirá un anuncio" + gating en asesor/resumen); falta publicar OTA a preview + validación en device + Play Console). Historial detallado por sesión/sprint en el git log.*
