...
- **🔴 Pendiente para activar la feature (retomar acá):** (1) **setear secrets** `RESEND_API_KEY`/`RESEND_FROM` (`bunx supabase secrets set`; crear cuenta Resend; verificar dominio para prod — hasta entonces las edges responden 502 "RESEND_API_KEY missing" pero NO rompen el alta de miembros). (2) **validar en device** flujo híbrido (A crea grupo → invita B por mail → B se une) + splits + settle + mail. (3) OTA a preview (sin deps nativas nuevas → OTA-safe). Quedan commits locales sin pushear (push lo corre el user). Opcional: migrar los hooks de `gdb` a cliente tipado (ya hay types).
