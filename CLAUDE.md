...
- **EAS `projectId` ausente** en `app.json/extra/eas`. Se autocompleta con `eas init` (interactivo — lo corre el user). Sin esto, `eas build` falla. Para un **APK funcional**: (1) `eas login` + `eas init`; (2) setear `EXPO_PUBLIC_SUPABASE_ANON_KEY` para el build (vía `eas env:create --environment preview` o agregándola al `env` de `eas.json` — es publishable); (3) `eas build -p android --profile preview` (genera APK gracias a `buildType: apk`). Iconos por default (los reales son cosméticos, no bloquean). `expo-updates` sigue removido → el APK no tiene OTA (no bloquea).
