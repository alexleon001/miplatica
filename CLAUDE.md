...
- [x] **Frontend polish** — `usePullRefresh` (refetch de queries activas) + `RefreshControl` en Dashboard/Movimientos/Inversiones; saludo del Dashboard usa `profile.name` (dato del onboarding) en vez del email; estados de error con reintento (`components/StateMessage.tsx`) en patrimonio, movimientos e inversiones; labels de accesibilidad en los FAB.
