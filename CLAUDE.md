...
> - **✅ TODO ENTREGADO + validado parcialmente en device:** pusheado a `main` (hasta `30eadd3`) + edge `monthly-summary` DEPLOYADO (ACTIVE v1, vía MCP) + **2 OTAs al canal `preview`** (runtime `0.1.0`, android+ios): `956c74b6` (sesión 8 completa, commit `5f2665b`) + **`2158d13c` (fix de FABs, commit `30eadd3`)**. El celular levanta reabriendo 2 veces.
> - **🐛 Bugs encontrados en device (validación 4/6) y YA ARREGLADOS (commit `30eadd3`, OTA `2158d13c`):** (a) en **Movimientos** el FAB "+ Nuevo" tapaba el "Total del mes" del desglose → el desglose + banner de recurrentes se movieron al `ListHeaderComponent` del FlatList (scrollean y libran el FAB); (b) en **Proyección** el FAB "+ Gasto" se solapaba con la barra de navegación del sistema → `Fab` ahora acepta `bottomInset` y la pantalla le pasa el safe-area inferior (+ se sacó el `edges=["bottom"]` redundante). **Falta confirmar ambos fixes en device.**
...
> - **🔴 Confirmar en device el OTA `2158d13c`:** (a) Movimientos — desglose expandido: "Total del mes" se ve completo, FAB no lo tapa; (b) Proyección — "+ Gasto" despegado de la barra del sistema.
> - **🔴 Validar el resto de la sesión 8** (OTA aplicado): proyección (auto-split cuotas, editar/borrar, checklist pagado, compartir, duplicar), alertas de presupuesto (80/100% + banner dashboard), **resumen mensual IA** (¡probar que el edge responda!), sparkline de patrimonio (≥2 días de aperturas), desglose por categoría, preferencias de notif, búsqueda, export CSV, recurrentes.
> - **Observación del user (4/6):** tiene **44 movimientos sin categoría** y "Otros" = 94% del gasto → sugerir tocar "Categorizar con IA" (`categorize-batch`) para que el desglose y el resumen IA sean útiles.
> - **Gotcha recurrentes:** el mes que los creás no aparecen en el banner (ya quedan registrados); el banner los ofrece desde el mes siguiente. **Gotcha sparkline:** arranca vacío hasta tener 2 días de snapshots. **Gotcha export CSV:** va como texto por el share sheet (archivo real = dep nativa futura).
