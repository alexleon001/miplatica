...
## Estado actual — sesión 17 (2026-06-16)
...
### s16 (2026-06-15) — puente rewarded→IA

**Foco: cerrar el puente Free→Pro de rewarded ads. El server-side YA estaba live (los docs lo daban por "pendiente" por error); faltaba el client wiring.**

- **✅ Crédito de IA por rewarded ad — COMPLETO end-to-end.**
  - **Server-side (ya estaba aplicado/deployado de s15, docs desactualizados):** migración `0013_ai_reward_credits` **aplicada** (tabla `ai_reward_credits` RLS owner-read + `grant_ai_reward_credit()`/`consume_ai_reward_credit()` `security definer`; tope 3/día y 3 acumulados; **sin SSV de AdMob** → un cliente malicioso se auto-otorga créditos pero el costo queda topeado a 3 llamadas/usuario/día = centavos) + las **4 edges de IA redeployadas** con el `ai-gate` que consume 1 crédito antes del 402 (`financial-advisor` v11 · `monthly-summary` v4 · `categorize-batch` v4 · `categorize-transaction` v10, verificadas vía MCP).
  - **Client wiring (commit `2c186f6`, ESTE sprint):** `lib/hooks/use-reward-credits.ts` (lee créditos; `watchAdForCredit()` = `showRewardedAd()`→`grant_ai_reward_credit()`; `invalidateRewardCredits()` que las pantallas llaman tras cada uso) + `ProLock` con CTA secundario "mirá un anuncio y usala una vez" + gating en `advisor`/`monthly-summary` (`isPro || créditos > 0`; advisor muestra hint en el composer al quedarse sin crédito y refresca el saldo tras cada respuesta; monthly-summary mantiene el resumen ya generado). Todo **OTA-safe / falla cerrado** (sin migración → 0 créditos; sin módulo nativo/unit → opción oculta). **En production la opción está OCULTA** (unit rewarded sin setear); solo aparece en `preview` (unit `test`).
  - **Chip de saldo** (commit `6b6dec2`): `components/RewardCreditsChip.tsx` muestra "Te quedan N usos de IA gratis" en asesor/resumen para usuarios Free con créditos (nada si es Pro o 0).
- **⏳ SSV de rewarded ads — foundation lista, SIN aplicar/deployar** (commit de esta sesión): migración `0014_ai_reward_credit_ssv` (`grant_ai_reward_credit_for(uuid)` solo service_role) + edge `rewarded-ssv` (`verify_jwt=false`, verifica firma ECDSA de AdMob contra `gstatic.com/admob/reward/verifier-keys.json` → otorga al `user_id` del callback). **Por qué no se activó:** SSV necesita un rewarded unit REAL en AdMob (production sin setear) + configurar la URL como SSV callback en ese unit — **imposible con test units** (no son tuyos), así que no es testeable todavía. **Activar SSV (cuando exista el unit real):** (1) `supabase` apply migración `0014`; (2) deployar edge `rewarded-ssv` (`verify_jwt=false`); (3) en AdMob console, pegar la URL `…/functions/v1/rewarded-ssv` como SSV callback del unit; (4) en `lib/rewarded.ts` setear `setServerSideVerificationOptions({ userId: supabaseUserId })` (pasar el id desde `use-reward-credits`); (5) **quitar** la llamada client-side a `grant_ai_reward_credit()` en `use-reward-credits` (el SSV la reemplaza → si no, doble crédito); (6) validar contra un callback real (el verificador ECDSA no se pudo probar acá).
- **✅ Interstitials con tope de frecuencia** (s15, commit `2de879d`): `lib/ad-frequency.ts` (1 cada 4 acciones, mín 4 min) + `lib/store/ads.ts` (persist) + `lib/interstitial.ts` (`maybeShowInterstitial(isPro)`). Trigger: guardar movimiento NUEVO en `add-transaction`. Pro / sin módulo / sin unit → no-op.
- **Env de ads:** `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ANDROID` / `_REWARDED_ANDROID` = `test` en `.env` + EAS `preview`; **production sin setear** (crear units reales en AdMob primero → capa apagada).
- **OTA:** ✅ **publicado a `preview` group `82828663`** (commit `63f1f83`, runtime 0.1.0): incluye el rewarded wiring + chip de créditos. Se aplica reabriendo la app 2 veces (matándola de verdad). ← `fbc6dcd1` (s15: interstitials + link privacidad).
- **Tests:** 125 `bun test` verdes · `type-check:app` limpio · `lint:app` 0 errores (5 warnings pre-existentes).
- **🔴 Pendiente (retomar acá):** (1) ~~validar en device en 2 tandas~~ **✅ HECHO (s17, 2026-06-16)** — Pro: sin ads + IA OK; Free: banner test + interstitial al 4° mov + rewarded→crédito IA + chip; Pro restaurado. (2) **Play Console**: subir AAB a track de testing interno + crear subs `pro_monthly` (base `monthly`, ~US$2,49) y `pro_annual` (base `annual`, ~US$19,99). (3) Menores: íconos definitivos · units reales de interstitial/rewarded en AdMob para production · **activar SSV** cuando exista el unit real (checklist arriba). Quedan commits locales sin pushear (push lo corre el user).
