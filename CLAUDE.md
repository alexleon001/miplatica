...
## Estado actual — sesión 16 (2026-06-15)
...
- **🔴 Pendiente (retomar acá):** (1) **validar en device en 2 tandas** (OTA `82828663` ya está; reabrir la app 2 veces) — tanda 1 como Pro (sin banner ni interstitials · asesor/resumen/categorización IA funcionan · de paso: movimientos por fecha + filtro categoría + link privacidad en Más); tanda 2 como Free: correr `update entitlements set is_pro=false where store='manual'` (vía MCP), matar/reabrir → banner test al pie + **interstitial test tras guardar el 4° movimiento** + 402→paywall en IA + **probar el rewarded: "mirá un anuncio" en el ProLock del asesor/resumen → anuncio test → desbloquea 1 uso de IA + aparece el chip "Te queda 1 uso gratis"**; **al final restaurar** `is_pro=true`. (2) **Play Console**: subir AAB a track de testing interno + crear subs `pro_monthly` (base `monthly`, ~US$2,49) y `pro_annual` (base `annual`, ~US$19,99). (3) Menores: íconos definitivos · units reales de interstitial/rewarded en AdMob para production · **activar SSV** cuando exista el unit real (checklist arriba). Quedan commits locales sin pushear (push lo corre el user).
...
| 10 | Monetización (freemium ads + Pro con IA, RevenueCat) + fixes device | 🚧 Fase 1 ✅ (s12) · Fase 2 código+build+webhook end-to-end ✅ + privacy policy live (s14) + interstitials (s15) + **crédito rewarded→IA end-to-end ✅ (s16)**; falta subs en Play Console + OTA del rewarded wiring + validar device |
