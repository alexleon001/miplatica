...
- [x] **Frontend polish** — `usePullRefresh` (refetch de queries activas) + `RefreshControl` en Dashboard/Movimientos/Inversiones; saludo del Dashboard usa `profile.name` (dato del onboarding) en vez del email; estados de error con reintento (`components/StateMessage.tsx`) en patrimonio, movimientos e inversiones; **skeleton loaders** animados (`components/Skeleton.tsx`: `Skeleton` + `RowsSkeleton`, pulso con Animated sin deps) en NetWorthCard/AccountsList/Movimientos/Inversiones; labels de accesibilidad en los FAB.
- [x] **Tab Deudas (Sprint 6 adelantado, read+create)** — `lib/hooks/use-debts.ts` (`useDebts` lista activas + `useCreateDebt` invalida debts + net_worth), `components/DebtItem.tsx`, pantalla `debts.tsx` (total adeudado + lista + skeleton/error/pull-refresh + FAB), modal `add-debt.tsx` (nombre/tipo/moneda/total/saldo/tasa/cuota/vencimiento). Resta al patrimonio neto vía `v_net_worth`. Falta: editar/pagar deuda + recordatorios de vencimiento.
- [x] **Prep EAS Build APK** — `eas.json` perfil `preview` ahora con `android.buildType: "apk"` + `env` (URL pública + APP_ENV). `app.json` ya tiene `android.package`/`bundleIdentifier` (`com.alexleon001.miplatica`).
- [x] **CRUD editar/borrar** — Borrar (long-press + confirmación, `lib/confirm.ts`) en movimientos/inversiones (hard delete) y cuentas/deudas (soft `is_active=false`, evita el FK restrict). Editar: cuentas y deudas reutilizan su modal de alta en modo edición (param `id`, prefill desde cache, `useUpdateAccount`/`useUpdateDebt`); tap = editar, long-press = borrar. **Pendiente:** editar movimientos e inversiones (hoy: borrar + recrear; el edit de esos modales —AI flow / re-derive— queda como follow-up).
- [x] **Presupuestos vivos** — migración `0005_budget_spent_triggers.sql` (trigger `sync_budget_spent` AFTER ins/upd/del en transactions + `init_budget_spent` BEFORE insert en budgets + backfill; mantiene `budgets.spent_ars` por owner/categoría/mes). Hook `useCreateBudget` (upsert por la unique), modal `add-budget.tsx`, CTA "+ Agregar presupuesto" en `BudgetsList`. `useCreateTransaction`/`useImportTransactions` ahora invalidan `budgets`. **Migración pendiente de aplicar (OK del user).**
- [x] **Tests de lógica financiera** (`bun test lib`, script `test:unit`) — 13 tests verde sobre `csv.ts` (parser), `broker-import.ts` (parseAmount AR/US, clasificación, dedup, columnas faltantes) y `instruments.ts` (`deriveInvestmentValues`: mercado ARS/USD, plazo fijo devengado, dólar MEP). `*.test.ts` excluido del tsc de app.
...
- **EAS `projectId` ausente** en `app.json/extra/eas`. Se autocompleta con `eas init` (interactivo — lo corre el user). Sin esto, `eas build` falla. Para un **APK funcional** (deps ya alineadas a SDK 54 + worklets ✅): (1) `eas login` + `eas init`; (2) setear `EXPO_PUBLIC_SUPABASE_ANON_KEY` para el build (vía `eas env:create --environment preview` o agregándola al `env` de `eas.json` — es publishable); (3) `eas build -p android --profile preview` (genera APK gracias a `buildType: apk`). Iconos por default (los reales son cosméticos, no bloquean). `expo-updates` sigue removido → el APK no tiene OTA (no bloquea).
