...
- **OTA:** último a `preview` = group `fbc6dcd1` (s15, commit `2de879d`: interstitials + link privacidad). **El client wiring del rewarded (commit `2c186f6`) todavía NO salió en un OTA** → para validarlo en device hay que publicar OTA a `preview` (o esperar al próximo build).
- **Tests:** 125 `bun test` verdes · `type-check:app` limpio · `lint:app` 0 errores (5 warnings pre-existentes).
- **🔴 Pendiente (retomar acá):** (1) **publicar OTA a `preview`** con el rewarded wiring (commit `2c186f6`) para poder validarlo en device. (2) **validar en device en 2 tandas** — tanda 1 como Pro (sin banner ni interstitials · asesor/resumen/categorización IA funcionan · de paso: movimientos por fecha + filtro categoría + link privacidad en Más); tanda 2 como Free: correr `update entitlements set is_pro=false where store='manual'` (vía MCP), matar/reabrir → banner test al pie + **interstitial test tras guardar el 4° movimiento** + 402→paywall en IA + **probar el rewarded: "mirá un anuncio" en el ProLock del asesor/resumen → anuncio test → desbloquea 1 uso de IA**; **al final restaurar** `is_pro=true`. (3) **Play Console**: subir AAB a track de testing interno + crear subs `pro_monthly` (base `monthly`, ~US$2,49) y `pro_annual` (base `annual`, ~US$19,99). (4) Menores: íconos definitivos · units reales de interstitial/rewarded en AdMob para production. Quedan commits locales sin pushear (push lo corre el user).
