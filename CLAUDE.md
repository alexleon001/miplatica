...
- **Plazo fijo (interés devengado):** ✅ mitigado en la pantalla de Inversiones — `freshenPlazoFijo` (en `use-investments.ts`) recalcula `current_value_*`/`profit_loss_*` al vuelo en cada render con la fecha de hoy (reusa `deriveInvestmentValues`). ⚠️ **Pendiente:** el patrimonio neto del dashboard (`v_net_worth`, server-side) sigue usando el valor guardado hasta que corre el cron → el plazo fijo ahí puede verse con ~1 día de atraso.
