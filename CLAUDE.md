...
- **Entregas:** OTA `2acef385` a `preview` (rebrand JS) · APK preview con nombre nativo "Mi Plata" buildeado en EAS (ver expo.dev/…/builds).
- **🔴 Pendiente s20+:** push a `main` (user; publica landing + CI) · instalar APK nuevo y **validar en device** (VE + Linea + splits + rebrand, backlog s7–s19) · Play Console (crear app, subs, custom listings AR/VE) · units reales AdMob + SSV · secrets Resend · screenshots/ícono definitivos · decidir dominio propio (¿miplata.app?).
