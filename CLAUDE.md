...
## Estado actual — sesión 15 (2026-06-12)

**Foco: interstitial + rewarded (JS puro, OTA-safe) + OTA a `preview` con el link de privacidad que faltaba.**

- **✅ Interstitials con tope de frecuencia** (commit `2de879d`): `lib/ad-frequency.ts` (lógica PURA testeable: 1 cada 4 acciones que califican, mínimo 4 min entre anuncios; constantes ahí) + `lib/store/ads.ts` (Zustand persist: `events`/`lastShownAt` sobreviven reinicios) + `lib/interstitial.ts` (`maybeShowInterstitial(isPro)`: precarga lazy recién cuando el próximo evento puede mostrar, show con delay 400 ms post-cierre de modal). Trigger único por ahora: guardar movimiento NUEVO en `add-transaction` (no edición). Pro / sin módulo nativo / sin unit ID → no-op.
- **✅ Helper de rewarded** `lib/rewarded.ts` (`showRewardedAd()` → `earned|dismissed|unavailable`, carga on-demand, timeout 15 s). **SIN UI que lo llame a propósito:** el puente "mirá un anuncio = 1 uso de IA" necesita crédito server-side en `ai-gate` (migración + redeploy, requiere OK del user) — sin eso el edge sigue dando 402 y el anuncio no compraría nada. Cablear UI junto con ese cambio.
- **Env nuevas:** `EXPO_PUBLIC_ADMOB_INTERSTITIAL_ANDROID` / `EXPO_PUBLIC_ADMOB_REWARDED_ANDROID` (misma semántica que banner: literal `test`=TestIds). Seteadas en `.env` local y EAS env `preview`=`test`; **production sin setear a propósito** (no existen los units reales en AdMob → capa apagada hasta que el user los cree).
- **✅ OTA publicado a `preview`** (group `fbc6dcd1`, commit `2de879d`): incluye el link de privacidad `16828d8` — que NO estaba ni en el APK `960d0f60` (buildeado antes de ese commit) ni en el OTA anterior (s11) — + interstitials. Se aplica reabriendo la app 2 veces (matándola de verdad).
- **Tests:** 125 `bun test` verdes (+5 de `ad-frequency`) · `type-check:app` limpio · `lint:app` 0 errores (5 warnings pre-existentes).
- **🔴 Pendiente (retomar acá):** (1) **validar en device en 2 tandas** — tanda 1 como Pro (sin banner ni interstitials · asesor/resumen/categorización IA funcionan · de paso: movimientos por fecha + filtro categoría + link privacidad en Más); tanda 2 como Free: correr `update entitlements set is_pro=false where store='manual'` (vía MCP), matar/reabrir la app → banner test al pie + **interstitial test tras guardar el 4° movimiento** + 402→paywall en IA + stub de compra (esperado sin subs en Play); **al final restaurar** `is_pro=true`. El atajo dev de paywall NO existe en el APK preview (solo `__DEV__`), por eso el flip por SQL. (2) **Play Console**: subir AAB a track de testing interno + crear subs `pro_monthly` (base `monthly`, ~US$2,49) y `pro_annual` (base `annual`, ~US$19,99) — recién ahí el paywall carga packages y la compra sandbox puebla `entitlements` vía el webhook ya verificado. (3) Menores: íconos definitivos · decidir/implementar crédito server-side del rewarded · units reales de interstitial/rewarded en AdMob para production. Quedan commits locales sin pushear (push lo corre el user).
...
| 10 | Monetización (freemium ads + Pro con IA, RevenueCat) + fixes device | 🚧 Fase 1 ✅ (s12) · Fase 2 código+build+webhook end-to-end ✅ + privacy policy live (s14) + interstitial/rewarded helper (s15); falta subs en Play Console + crédito server rewarded + validar device |
...
*Última actualización: 2026-06-12 (sesión 15: interstitials con tope + helper rewarded + OTA `fbc6dcd1` a preview con el link de privacidad; falta validación en device (2 tandas, flip Pro por SQL) + Play Console). Historial detallado por sesión/sprint en el git log.*
