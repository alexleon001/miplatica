...
## Estado actual — sesión 18 (2026-06-19)

**Foco s18: feature grande NUEVA — Gastos compartidos (split de gastos estilo Splitwise). PRIMER dominio multi-usuario de la app.** Código completo (Fases 1–5) + **server-side YA aplicado/deployado vía MCP**. Falta: secrets Resend + validar en device.

- **Decisiones de producto:** (1) modelo **híbrido** de participantes — invitás por email; si la persona tiene/baja la app se vincula a su cuenta real (ve el grupo), si no queda "fantasma" (nombre+email) y recibe el mail igual; al registrarse/loguearse con ese email se vincula solo. (2) **Free = 1 grupo activo** creado por vos (Pro = ilimitados; ser miembro de grupos ajenos es gratis). (3) Recordatorios **por mail (Resend)**. (4) **Ledger separado** del patrimonio: saldar puede registrar opcionalmente un movimiento real.
- **Backend (migración `0015_expense_groups`, ✅ APLICADA s18):** 5 tablas (`expense_groups`, `group_members` con `user_id` null=fantasma, `shared_expenses`, `expense_splits`, `settlements` con `recorded_transaction_id` → puente opcional al ledger). RLS por **membresía** vía `is_group_member()`/`is_group_owner()` **security definer** (clave: evita recursión de RLS). RPCs: `create_expense_group()` (alta atómica + **enforce límite Free** server-side, lanza `group_limit_reached`), `claim_group_invites()` (vincula fantasmas por email), `my_group_balances()` (saldo neto por grupo, 1 round-trip). `handle_new_user` extendido para claim al signup. Cron en `0016_schedule_split_reminders` (diario 12:00 UTC).
- **Lógica pura `lib/splits.ts` (+17 tests, 142 total verdes):** `splitEqual/Exact/byWeights` (centavos exactos), `computeBalances` (invariante suma=0), `simplifyDebts` (greedy, transferencias mínimas "quién le debe a quién"), `settlementText`.
- **Cliente:** hooks `use-groups`/`use-group`/`use-group-members`/`use-shared-expenses`/`use-settlements` (tablas nuevas vía `gdb` = cliente sin tipar en `lib/groups-types.ts`, hasta regenerar `database.types` post-apply). Stack `app/groups/` (`index` lista con saldo por grupo, `[id]` detalle: balances simplificados + acciones + historial + botón "Recordar"). Modales `add-group`/`add-shared-expense`/`settle-up`/`invite-member`. Entrada en **Más** + card `SharedExpensesCard` en dashboard. Claim de invites cableado en `_layout` post-login.
- **Email (Resend, infra NUEVA — antes no había envío):** `_shared/email.ts` (`sendEmail` + plantillas invite/reminder, falla cerrado sin key). Edges **✅ DEPLOYADAS s18**: `group-invite` (v1, jwt), `group-reminder` (v1, jwt, botón "Recordar" cuando sos acreedor), `split-reminders-cron` (v1, verify_jwt=false, throttle 6 días vía `last_reminded_at`). Cron programado `[0 12 * * *]`. **Secrets faltan:** `RESEND_API_KEY` + `RESEND_FROM` (los setea el user; en pruebas `onboarding@resend.dev`).
- **✅ Aplicado server-side vía MCP (s18):** migraciones `0015`+`0016` aplicadas (verificado: 5 tablas con RLS, 5 funciones, 10 policies, cron OK), 3 edges ACTIVE, `lib/database.types.ts` regenerado (incluye tablas + RPCs nuevas). 142 tests verdes, type-check + lint limpios.
- **OTA:** ✅ publicado a `preview` group `b5a48d65` (commit `989dec8`, runtime 0.1.0). Commit pusheado a `main` (bypass admin del check requerido; CI corre igual).
- **🔴 Pendiente para activar la feature (retomar acá):** (1) **setear secrets** `RESEND_API_KEY`/`RESEND_FROM` (`bunx supabase secrets set`; crear cuenta Resend; verificar dominio para prod — hasta entonces las edges responden 502 "RESEND_API_KEY missing" pero NO rompen el alta de miembros). (2) **validar en device** flujo híbrido (A crea grupo → invita B por mail → B se une) + splits + settle + mail (reabrir la app 2 veces para tomar el OTA). Opcional: migrar los hooks de `gdb` a cliente tipado (ya hay types).
