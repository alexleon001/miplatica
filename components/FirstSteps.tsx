    ...
    <View style={styles.card}>
      <View style={styles.header}>
        <View style={styles.titleRow}>
          <Ionicons name="rocket-outline" size={16} color={colors.primaryBright} />
        ...
        {steps.map((step) => (
          ...
          >
            <Ionicons
              ...
              size={22}
              color={step.done ? colors.positive : colors.textMuted}
            />
            <Text style={[styles.stepLabel, step.done && styles.stepLabelDone]} numberOfLines={1}>
              {step.label}
            </Text>
            {step.done ? null : <Ionicons name="chevron-forward" size={18} color={colors.textMuted} />}
          </Pressable>
        ))}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  stepLabel: { ...typography.body, color: colors.textPrimary, flex: 1, fontWeight: "600" },
  stepLabelDone: { color: colors.textMuted, textDecorationLine: "line-through", fontWeight: "500" },
});
